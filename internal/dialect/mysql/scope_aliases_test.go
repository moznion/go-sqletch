...
func TestColumnRefs_ScopeAliases_InTestExprStaysOuter(t *testing.T) {
	// The left operand of IN is in the enclosing scope, not the subquery.
	sql := "SELECT u.id FROM users u JOIN orgs o ON o.id = u.org_id " +
		"WHERE o.id IN (SELECT x.oid FROM audits o)"
	test, ok := findScope(parse(t, sql).ColumnRefs(), "o", "id")
	if !ok {
		t.Fatalf("no o.id test-expr ref")
	}
...
// that uses it, so the using-select's FROM name (here the CTE name c)
// must NOT be inherited into the CTE body's scope.

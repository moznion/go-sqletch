...
func TestGenerate_UseCase1(t *testing.T) {
	...
	for _, want := range []string{
		...
		`OrganizationID\s+sqletch\.Omittable\[int64\]`,
		`Status\s+sqletch\.Omittable\[string\]`,
		`arg\.OrganizationID\.IsPresent\(\)`,
		`arg\.OrganizationID\.Ptr\(\), arg\.Status\.Ptr\(\), arg\.EmailPrefix\.Ptr\(\), arg\.Limit`,

...
`)
	...
	for _, dir := range []string{sqlDir, goDir} {
		...
		if code := cli.Generate(ctx, filepath.Join(dir, "sqletch.yaml"), false, cli.RunOptions{AllowDestructive: true}, &out, &errW); code != cli.ExitOK {
	...
	var out, errW bytes.Buffer
	if code := cli.Check(ctx, filepath.Join(goDir, "sqletch.yaml"), false, false, cli.RunOptions{AllowDestructive: true}, &out, &errW); code != cli.ExitOK {

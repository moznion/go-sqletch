...
rows, err := q.SearchUsers(ctx, gen.SearchUsersParams{
    Status: sqletch.Present("active"),       // the zero value omits the predicate
...
in *all* shapes. Parameters written into nullable columns are
`Option[T]` too (`None` writes NULL), distinct from the
`sqletch.Omittable[T]` of optional fragments. Planner-only failures (e.g. `FOR UPDATE` with an

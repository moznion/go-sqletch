...
| SQLETCH306 | `cache.path` or `output.path` escapes the project directory. A **relative** path climbing out with `..` is an **error** (refused: a cloned repository could otherwise redirect sqletch's writes to arbitrary locations); an **absolute** path is a **warning** (a deliberate operator choice — generation still proceeds). Keep the path inside the project to silence it. |

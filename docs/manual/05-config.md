...
- **`database.oracle`** selects the type-oracle backend: `server`
  ...
  config error), currently **beta**. See
  [Dialects](04-dialects.md#the-native-oracle-backend-databaseoracle-native--beta)
  for the discipline it demands, the beta caveat, and what
  `check --exhaustive` proves under it.
